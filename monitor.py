...
root = tk.Tk()
root.withdraw()
API_KEY = "d70a3e867f6cc6b9f79b504270412b543fabed8f87d9a760d614a05205d73fad16c0e4072b9441bc"
#API_KEY = simpledialog.askstring("API KEY", "Inserisci la tua API KEY:", parent=root)
THRESHOLD = simpledialog.askinteger("THRESHOLD", "Inserisci il valore del THRESHOLD:", parent=root)
...
def check_ip(ip, process, pid):
    try:
        ...
        if score >= THRESHOLD:
            with open("bad_ip_trust.txt", "r") as f:
                bad_ip_list = f.read().splitlines()
#bad_ip_list = f.read().splitlines() DA QUESTO RISULTATO:
#['103.162.20.168:System:0:2023-01-22 21:23:21']
#bad_ip_list deve contenere solamente l'ip , il primo numero prima dei due punti :
                print(bad_ip_list)
            if ip.split(':')[0] in bad_ip_list:
                print(f"IP {ip.split(':')[0]} already present in bad_ip_trust.txt, skipping termination process")
            else:
                with open("ip_bad.txt", "a") as f:
                    f.write(ip.split(':')[0] + ':' + process + ':' + pid + "\n")
                    ip_bad.add(ip)
                    result = messagebox.askquestion("Terminate process", f"Do you want to terminate the process with PID {pid} il processo è {process}. Score abuse: {score}", icon='warning')
                    if result == 'no':
                        from datetime import datetime
                        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        with open("bad_ip_trust.txt", "w") as f:
                            f.write(ip.split(':')[0] + ':' + process + ':' + pid + ':' + current_time + "\n")
                    elif result == 'yes':
                        subprocess.run(["taskkill", "/PID", pid, '/F'], stdout=subprocess.PIPE)
                        with open("ip_bad.txt", "a") as f:
                            f.write(ip.split(':')[0] + ':' + process + ':' + pid + ':' + 'terminato' + "\n")
                            print(f"Process with PID {pid} and IP {ip.split(':')[0]} has been terminated and added to ip_bad.txt with status terminato")
                    else:
                        print("Invalid input, process not terminated")
        else:
